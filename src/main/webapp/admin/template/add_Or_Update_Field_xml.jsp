...
<%
	...
	if ("add".equals(flag)) {
		...
		String[] controlOptionIdStrArray = (String[]) ArrayUtil
				.format(request.getParameterValues("controlOptionId"), new String[0]);
		...
		//control role ids
		String[] controlRoleIdArray = (String[]) ArrayUtil.format(request.getParameterValues("controlRoleId"),
				new String[0]);
		field.setControlRoleIds(new LinkedHashSet<String>(Arrays.asList(controlRoleIdArray)));

		//control action ids
		String[] controlActionIdArray = (String[]) ArrayUtil
				.format(request.getParameterValues("controlActionId"), new String[0]);
		...
		String[] actionIdStrArray = (String[]) ArrayUtil.format(request.getParameterValues("actionId"),
				new String[0]);
		...
		//先添加字段，再更新表单
		String fieldColName = FieldNameMapMySQL.getInstance().getOneFieldName(field,
				template.getId().getValue());
		if (fieldColName == null || fieldColName.length() == 0) {
			out.println(ErrorManager.getErrorXml(ErrorType.field_update_error));
			return;
		} else {
			if (das.addFieldColName(template.getId().getValue(), fieldColName, field.getId().getValue(),
					FieldNameMapMySQL.getInstance().getFieldColNameType(field))) {
